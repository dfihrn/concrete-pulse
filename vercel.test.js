...
test('a true concurrent writer that advances collectedAt returns 200 skipped',async()=>{const f=fixture();let release,entered=0;const gate=new Promise(r=>release=r);const h=createVercelHandlers({secret,storeFactory:()=>f.store,fetchCurrentSnapshot:async()=>{entered++;if(entered===2)release();await gate;return snapshot();}});const results=await Promise.all([invoke(h.collect),invoke(h.collect)]);assert.equal(results.filter(r=>r.body.status==='collected').length,1);const skipped=results.find(r=>r.body.reason==='concurrent-collection');assert.ok(skipped);assert.equal(skipped.statusCode,200);assert.equal((await f.store.load()).revision,'1');});
...
test('a stale ETag without an advancing writer returns a non-2xx response',async()=>{const original=makeGeneration(null,snapshot(),'2026-09-11T00:00:00Z');const store={load:async()=>({generation:original,revision:'stale'}),commit:async()=>{throw new GenerationConflict('stale revision');}};const h=createVercelHandlers({secret,storeFactory:()=>store,now:()=>Date.parse('2026-09-11T00:10:00Z'),fetchCurrentSnapshot:async()=>snapshot(200)});const r=await invoke(h.collect);assert.equal(r.statusCode,409);assert.notEqual(r.body.status,'skipped');});
test('successful collections rotate current into previous and advance the ETag',async()=>{const f=fixture();let clock=Date.parse('2026-09-11T00:00:00Z');let tvl=100;const h=createVercelHandlers({secret,storeFactory:()=>f.store,now:()=>clock,fetchCurrentSnapshot:async()=>snapshot(tvl)});assert.equal((await invoke(h.collect)).statusCode,200);const first=await f.store.load();assert.equal(first.generation.previous,null);assert.equal(first.revision,'1');clock+=61000;tvl=200;assert.equal((await invoke(h.collect)).statusCode,200);const second=await f.store.load();assert.deepEqual(second.generation.previous,first.generation.current);assert.equal(second.generation.current.vaults[1]['0x'+'a'.repeat(40)].tvl,200);assert.equal(second.revision,'2');assert.notEqual(second.revision,first.revision);});
